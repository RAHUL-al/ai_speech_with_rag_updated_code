...
# Track filenames for later combination and cleanup
generated_files = []
...
for i, chunk in enumerate(chunks):
    ...
    for j, (gs, ps, audio) in enumerate(generator):
        filename = f'chunk_{i+1}_part_{j+1}.wav'
        display(Audio(data=audio, rate=24000, autoplay=i == 0 and j == 0))
        sf.write(filename, audio, 24000)
        generated_files.append(filename)

# Step 3: Combine all audio files into one
...
# Step 4: Delete all chunk files after combining
for file in generated_files:
    try:
        os.remove(file)
